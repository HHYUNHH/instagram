...
def collect_qrs(driver, target, log_list):
    ...
    for feed in dic:
        ...
        old = 0
        driver.get('https://www.instagram.com/' + target + dic[feed][0])
        while True:
            check_count = 0
            time.sleep(1)
            driver.implicitly_wait(5)
            tag = driver.find_elements(By.CSS_SELECTOR, f'{dic[feed][1]} .x1i10hfl')
            for i in tag:
                qrs = i.get_attribute('href').split('/')[-2]
                if qrs not in dic[feed][2]:
                    if qrs in log_list:
                        if check_count == 3:
                            old = len(dic[feed][2])
                            break
                        check_count += 1
                    dic[feed][2].append(qrs)
